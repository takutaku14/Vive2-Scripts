...
for i in range(10):
    ...
    # CSVファイルのパス
    file_path = rf'c:\Users\takut\OneDrive\ドキュメント\github\Vive2-Scripts\data\{i}\RightAverageList.csv'
...
for index, df in data:
    if 'Time' in df.columns and 'Average Diameter' in df.columns:
        ...
        # グラフの装飾
        plt.title(f'RightAverage - data {index}', fontsize=16)
        ...
        plt.savefig(os.path.join(
            output_dir, f'RightAverage_Data_{index}({names[index]}).png'))
        plt.show()
...
# まとめたグラフの装飾
plt.title('RightAverage - All Data', fontsize=16)
plt.xlabel('Time', fontsize=14)
plt.ylabel('Average Diameter', fontsize=14)
plt.grid(True, linestyle='--', alpha=0.7)
plt.legend(fontsize=12)
...
# まとめたグラフの保存
plt.savefig(os.path.join(output_dir, 'RightAverage_All.png'))
plt.show()
