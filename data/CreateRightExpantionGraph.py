...
for i in range(10):
    ...
    # CSVファイルのパス
    file_path = rf'c:\Users\takut\OneDrive\ドキュメント\github\Vive2-Scripts\data\{i}\RightExpantionList.csv'
    ...
    # グラフの装飾
    plt.title(f'RightExpantion - Data {i}', fontsize=16)
    ...
    plt.savefig(os.path.join(
        output_dir, f'RightExpantion_data_{i}({name}).png'))  # データ番号,名前を含む画像名で保存
    plt.show()
...
# まとめたグラフの装飾
plt.title('RightExpantion - All Data', fontsize=16)
plt.xlabel('Time', fontsize=14)
plt.ylabel('Diameter', fontsize=14)
plt.grid(True, linestyle='--', alpha=0.7)
plt.legend(fontsize=12)
...
# まとめたグラフの保存
plt.savefig(os.path.join(
    output_dir, 'RightExpantion_all.png'))  # まとめた画像名で保存
plt.show()
